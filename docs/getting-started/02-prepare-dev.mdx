...
> Note that all secrets stored ina vault are for a single environment. This means that if you have multiple environments (e.g., dev, staging, prod), you will need to create a separate vault for each environment.
