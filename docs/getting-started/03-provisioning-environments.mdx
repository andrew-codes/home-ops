...
```bash
yarn nx provision resource-k8s --env=$ENV
```
...
```bash
yarn nx provision resource-fluxcd --env=$ENV
```

## Deploying kubeseal

> This will automatically save the public key to disk. This files should be committed to the repository. These keys may be used to encrypt secrets, via the kubeseal CLI, to be committed to the repository.

```bash
yarn nx deploy resource-sealed-secrets --env=$ENV
```
